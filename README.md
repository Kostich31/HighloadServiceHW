...
![image](https://user-images.githubusercontent.com/29610387/201545978-e36b15ef-6541-4044-9aaa-8c0fee119e80.png)


# 4. Физическая схема БД
![image](https://user-images.githubusercontent.com/29610387/201546143-5d8b4c33-2c9a-49a7-891f-fa9f7f9942a3.png)
...
Worker service - сервис, позволяющий работать системе асинхронно.

![image](https://user-images.githubusercontent.com/29610387/201717043-c0f5aeef-533f-4451-84d0-8da9505831a1.png)
...
Тогда необходимо будет: 621 354 / 30 000 = 21 сервер

**Базы данных**

Посчитаем объем для 55 000 000 млн. пользователей с учетом 5-ти летнего периода.

Получаем: 5.1 гБ * 55 000 000 / 2^20 = 267.5 ПБ

Учитывая, что практически весь объем занимают медиафайлы, то будем считать количество бакетов, необходимое для Amazon S3:

Один бакет может хранить до 5 Тб [[9]](https://cloudian.com/blog/s3-storage-behind-the-scenes/), тогда 267.5 * 1024 / 5 = 54 784 бакетов

**PostgreSQL**

При правильной конфигурации [[10]](https://www.ashnik.com/fine-tuning-postgres-to-achieve-5000-queries-per-second/)  из PostrgreSQL можно получить 5000 QPS.

Учитывая, что почти все запросы требует похода в базу данных, то посчитать необходимое количество серверов можно из общего количества RPS получаем 124 сервера.
**Redis** 
